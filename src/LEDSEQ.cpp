...
LEDSEQWidget::LEDSEQWidget() {
	LEDSEQ *module = new LEDSEQ();
	setModule(module);
...

	{
		...
		panel->setBackground(SVG::load(assetPlugin(plugin, "res/LEDSEQ.svg")));
		addChild(panel);
	}
	...
	for (int i = 0; i < 16; i++) {
	for (int j = 0; j < 5; j++) {
     		addParam(createParam<LButton>(Vec(j*15+10-0.8, i*15+35-0.8+51), module, LEDSEQ::ON_PARAM + (i*5+j), 0.0, 1.0, 0.0));
	...
	addParam(createParam<LEDButton>(Vec(35, 340), module, LEDSEQ::EDIT_PARAM, 0.0, 1.0, 0.0));
