...
L3DS3QWidget::L3DS3QWidget() {
	L3DS3Q *module = new L3DS3Q();
	setModule(module);
...

	{
		...
		panel->setBackground(SVG::load(assetPlugin(plugin, "res/L3DS3Q.svg")));
		addChild(panel);
	}
	...
	for (int i = 0; i < 16; i++) {
	for (int j = 0; j < 5; j++) {
     		addParam(createParam<LButton>(Vec(j*15+10-0.8, (15-i)*15+15-0.8+51), module, L3DS3Q::ON_PARAM + (i*5+j), 0.0, 1.0, 0.0));
	...
	addParam(createParam<LEDButton>(Vec(65, 31), module, L3DS3Q::EDIT_PARAM, 0.0, 1.0, 0.0));
